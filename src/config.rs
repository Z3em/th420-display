...
pub fn interpolate_color(value: f32, map: &[ColorPoint]) -> [u8; 3] {
    ...
    if map.len() == 1 || value <= map[0].value {
        return map[0].color;
    }
    if value >= map.last().unwrap().value {
        return map.last().unwrap().color;
    }
...
impl SensorConfig {
    #[allow(dead_code)]
    pub fn label_color_f32(&self) -> [f32; 3] {
        to_f32(self.label_color)
    }
    #[allow(dead_code)]
    pub fn set_label_from_f32(&mut self, c: [f32; 3]) {
        self.label_color = from_f32(c);
    }
...
pub struct BackgroundConfig {
    ...
    /// Additional scale applied after the selected fit mode.
    ...
    pub opacity: u8,
    /// Background/canvas color exposed by Contain, zoom, and repositioning.
    #[serde(default = "default_background_color")]
    pub background_color: [u8; 3],
    /// Optional Gaussian blur in pixels.
    #[serde(default)]
    pub blur_sigma: f32,
}
...
fn grid_slots(ids: &[&str], cols: usize, rows: usize) -> Vec<ResolvedSlot> {
    let col_xs: Vec<i32> = match cols {
        ...
        3 => vec![110, 240, 370],
        _ => (0..cols)
            .map(|i| 90 + (i * (300) / cols.saturating_sub(1).max(1)) as i32)
            .collect(),
    };
    let row_ys: Vec<i32> = match rows {
        ...
        _ => (0..rows)
            .map(|i| 80 + (i * 320 / rows.saturating_sub(1).max(1)) as i32)
            .collect(),
    };
...
mod tests {
    ...
    fn migrate_adds_sensors_missing_from_old_config() {
        let mut cfg = Config::default();
        cfg.sensors
            .retain(|s| s.id != "igpu_temp" && s.id != "gpu_hotspot_temp");
